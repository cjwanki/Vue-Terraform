...
#   for_each = {
#     for dvo in aws_acm_certificate.vue-dev_cert.domain_validation_options : dvo.domain_name => {
...
# resource "aws_acm_certificate_validation" "vue-dev_cert_validation" {
#   certificate_arn         = aws_acm_certificate.vue-dev_cert.arn
...
resource "aws_instance" "vue-dev" {
  ...
  tags = { Name = "vue-dev-app-server" }
}

# Load Balancer for HTTP and HTTPS traffic
resource "aws_lb" "vue-dev_lb" {
  name               = "vue-dev-app-lb"
  internal           = false
  ...
  tags = { Name = "vue-dev-app-lb" }
}

# Target Group pointing to the EC2 instance
resource "aws_lb_target_group" "vue-dev_tg" {
  name        = "vue-dev-app-tg"
  port        = 80
  protocol    = "HTTP"
  vpc_id      = aws_vpc.vue-dev.id
  ...
  tags = { Name = "vue-dev-app-tg" }
}

# Register the EC2 instance with the Target Group
resource "aws_lb_target_group_attachment" "vue-dev_tg_attachment" {
  target_group_arn = aws_lb_target_group.vue-dev_tg.arn
  target_id        = aws_instance.vue-dev.id
  port             = 80
}

# HTTP Listener to redirect traffic to HTTPS
resource "aws_lb_listener" "vue-dev_http_listener" {
  load_balancer_arn = aws_lb.vue-dev_lb.arn
  port              = 80
  protocol          = "HTTP"
...
# resource "aws_lb_listener" "vue-dev_https_listener" {
#   load_balancer_arn = aws_lb.vue-dev_lb.arn
#   port              = 443
#   protocol          = "HTTPS"
...
resource "aws_route53_record" "vue-dev_record" {
  zone_id = aws_route53_zone.mycloudlab_zone.zone_id
  name    = "mycloudlab.space"
  type    = "A"
  alias {
    name                   = aws_lb.vue-dev_lb.dns_name
    zone_id                = aws_lb.vue-dev_lb.zone_id
