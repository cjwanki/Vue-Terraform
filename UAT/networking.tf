...
#   for_each = {
#     for dvo in aws_acm_certificate.vue-uat_cert.domain_validation_options : dvo.domain_name => {
...
# resource "aws_acm_certificate_validation" "vue-uat_cert_validation" {
#   certificate_arn         = aws_acm_certificate.vue-uat_cert.arn
...
resource "aws_instance" "vue-uat" {
  ...
  tags = { Name = "vue-uat-app-server" }
}

# Load Balancer for HTTP and HTTPS traffic
resource "aws_lb" "vue-uat_lb" {
  name               = "vue-uat-app-lb"
  internal           = false
  ...
  tags = { Name = "vue-uat-app-lb" }
}

# Target Group pointing to the EC2 instance
resource "aws_lb_target_group" "vue-uat_tg" {
  name        = "vue-uat-app-tg"
  port        = 80
  protocol    = "HTTP"
  ...
  tags = { Name = "vue-uat-app-tg" }
}

# Register the EC2 instance with the Target Group
resource "aws_lb_target_group_attachment" "vue-uat_tg_attachment" {
  target_group_arn = aws_lb_target_group.vue-uat_tg.arn
  target_id        = aws_instance.vue-uat.id
  port             = 80
}

# HTTP Listener to redirect traffic to HTTPS
resource "aws_lb_listener" "vue-uat_http_listener" {
  load_balancer_arn = aws_lb.vue-uat_lb.arn
  port              = 80
  protocol          = "HTTP"
...
# resource "aws_lb_listener" "vue-uat_https_listener" {
#   load_balancer_arn = aws_lb.vue-uat_lb.arn
#   port              = 443
#   protocol          = "HTTPS"
...
resource "aws_route53_record" "vue-uat_record" {
  zone_id = aws_route53_zone.mycloudlab_zone.zone_id
  name    = "mycloudlab.space"
  type    = "A"
  alias {
    name                   = aws_lb.vue-uat_lb.dns_name
    zone_id                = aws_lb.vue-uat_lb.zone_id
    evaluate_target_health = true
  }
}

# If the second record is needed, rename it to a unique name
resource "aws_route53_record" "vue-uat_record_alt" {
  zone_id = aws_route53_zone.mycloudlab_zone.zone_id
  name    = "alt.vue.com" # Use a different subdomain or name
  type    = "A"
  alias {
    name                   = aws_lb.vue-uat_lb.dns_name
    zone_id                = aws_lb.vue-uat_lb.zone_id
